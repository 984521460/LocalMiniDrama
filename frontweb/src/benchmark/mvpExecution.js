...
const TRUSTED_PROGRESS = new WeakSet()
...
const PROGRESS_KEYS = FREEZE([
  'schemaVersion', 'authorizationUid', 'sessionUid', 'dramaUid', 'completedCount',
  'totalCount', 'batchComplete', 'nextItem', 'batchSha256',
])
const NEXT_ITEM_KEYS = FREEZE(['ordinal', 'itemKind', 'itemUid'])
...
function projectProgress(value, sessionValue, authorizationValue, batchValue) {
  const batch = mvpBenchmarkPreflightBatchView(
    batchValue, sessionValue, authorizationValue,
  )
  const input = exactObject(value, PROGRESS_KEYS)
  const completedCount = safeInteger(input.completedCount, 0, 64)
  const totalCount = safeInteger(input.totalCount, 2, 64)
  const batchComplete = input.batchComplete
  if (input.schemaVersion !== 'mvp-benchmark-production-execution-progress.v1'
    || input.authorizationUid !== batch.authorizationUid
    || input.sessionUid !== batch.sessionUid
    || input.dramaUid !== batch.dramaUid
    || input.batchSha256 !== batch.batchSha256
  ...
  let nextItem = null
  if (batchComplete) {
    if (input.nextItem !== null) invalid()
  } else {
    const itemInput = exactObject(input.nextItem, NEXT_ITEM_KEYS)
    const ordinal = safeInteger(itemInput.ordinal, 0, 63)
    const reservation = batch.reservations[ordinal]
    if (ordinal !== completedCount || !reservation
      || itemInput.itemKind !== reservation.itemKind
      || itemInput.itemUid !== reservation.itemUid) invalid()
    nextItem = FREEZE({
      ordinal,
      itemKind: itemInput.itemKind,
      itemUid: itemInput.itemUid,
...
function trustProgress(value) {
  REFLECT_APPLY(WEAK_SET_ADD, TRUSTED_PROGRESS, [value])
  return value
}
...
export function parseMvpBenchmarkProductionExecutionProgressJson(
  text, sessionValue, authorizationValue, batchValue,
) {
  return trustProgress(projectProgress(
    parseStrictJson(text), sessionValue, authorizationValue, batchValue,
  ))
}

export function mvpBenchmarkProductionExecutionProgressView(
  value, sessionValue, authorizationValue, batchValue,
) {
  if (!value || typeof value !== 'object'
    || !REFLECT_APPLY(WEAK_SET_HAS, TRUSTED_PROGRESS, [value])) invalid()
  return trustProgress(projectProgress(value, sessionValue, authorizationValue, batchValue))
}
