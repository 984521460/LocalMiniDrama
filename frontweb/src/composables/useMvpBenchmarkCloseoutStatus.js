...
import { mvpBenchmarkCloseoutStatusAPI } from '../api/v2/mvpBenchmarkCloseoutStatus.js'
import { mvpBenchmarkCloseoutStatusView } from '../benchmark/mvpCloseoutStatus.js'
...
const ERROR_CODE = 'MVP_BENCHMARK_CLOSEOUT_STATUS_REQUEST_FAILED'

export function createMvpBenchmarkCloseoutStatusState({ getStatus }) {
  if (typeof getStatus !== 'function') {
    throw new TypeError('MVP benchmark closeout status state is invalid')
  }
  ...
  async function load(sessionValue, authorizationValue, batchValue) {
    ...
    try {
      ...
      const result = mvpBenchmarkCloseoutStatusView(
        await getStatus(session, authorization, batch), session, authorization, batch,
      )
...
export function useMvpBenchmarkCloseoutStatus({ api = mvpBenchmarkCloseoutStatusAPI } = {}) {
  return createMvpBenchmarkCloseoutStatusState({
