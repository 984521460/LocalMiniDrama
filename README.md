...
v2 工作流画布支持把当前已批准的原文选区、事实提取、单集改编和结构化剧本绑定为一张类型安全的连线图，保存后可原样重开，并由后端调度器按拓扑执行。每个下游证据节点都会重新核对实际运行输出是否正是自身批准链的直接上游；仅类型相容但来源错绑的连线会在节点成功前失败关闭，不会重新调用模型或生成媒体。

成片编译前可在最终导出面板中按完整镜头列表执行上移、下移或恢复原顺序。提交时前端发送同一冻结 MVP 会话内 H3 task UID 的无重复、无遗漏排列；后端把排列重新绑定到可信镜头、对白音频和字幕，使用 `audio-timeline-ms.v3` 生成按成片时间排序的音频/字幕段，并把顺序封存在 `export.final` 输入和最终执行计划中。旧 v1 成片请求与 v1/v2 音频时间线仍兼容；非法或不同的并发排列会在媒体探测和状态写入前拒绝。
