...
} = require('../src/services/projectArchiveV21ExportService');
...
test('production candidate execution makes four independent calls and seals one durable batch', async (t) => {
  ...
  assert.throws(
    ...
    /does not yet preserve character candidate execution evidence/u,
  );
  assert.throws(
    () => createProjectArchiveV21ExportService().exportDrama({
      database,
      storageRoot: root,
      dramaId: 1,
    }),
...
test('provider uncertainty is persisted and installed partial media is removed', async (t) => {
  ...
  await assert.rejects(service.execute(executionRequest(ids, uid(31011))), (error) => (
